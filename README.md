...
| `taobao.urls.file` | 无 | 启动即抓的链接列表，等价于把路径作为第一个命令行参数 |

系统属性只在 `CrawlConfig` / `ChromeConfig` / `CliOptions` 三处读取，其余代码一律拿配置对象。
加新配置项改这三个类之一，不要在业务代码里再写 `System.getProperty`。

## 代码结构

面向 **Spring Boot 三层架构** 演进，当前 CLI 版已按层分包（不是单包扁平，也不是过度碎片化）：

| 当前包 | 职责 | 上 Spring Boot 后 |
|---|---|---|
| `cli` | 控制台入口、人机交互 | → `controller`（REST / WebSocket） |
| `service` | 业务编排：`CrawlService`、进度回调、`HumanGate` | → `@Service` |
| `repository` | 结果持久化：`ResultRepository`、`FileResultRepository`、`OutputWriter` | → `@Repository` / MyBatis `mapper` |
| `crawler` | 单条抓取引擎 `ItemCrawler` | → 基础设施 Bean |
| `crawler.browser` | Selenium：Chrome、页面操作、点选补价 | 不变 |
| `crawler.parser` | 页面数据 → `ItemDetail`、链接解析 | 不变 |
| `config` | `CrawlConfig`、`ChromeConfig`、`Constants` | → `@ConfigurationProperties` |
| `model` | 纯数据与枚举（**只有取值，不含业务逻辑**） | entity / DTO |
| `tool` | 离线自检 / 复算 | 独立保留 |

业务逻辑不放 enum 里，而是放在对应职责的类中：

| 逻辑 | 放在哪 |
|---|---|
| 构造 `CrawlResult` | `service.CrawlResults` |
| 判断价格精度 | `repository.OutputWriter#priceSourceOf` |
| 从 URL 判断平台 | `crawler.parser.ItemLinks#platformOfUrl` |

`HumanGate` / `ResultRepository` 是两个关键扩展点：CLI 用 stdin + 写文件，服务化换成任务状态机 + 写库/写 OSS，**抓取核心不用改**。
...
```bash
mvn -B -q compile exec:java -Dmain.class=com.san.taobao.tool.ReprocessTool
```
